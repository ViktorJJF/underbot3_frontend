import api from '@/services/api/entities';
import {
  buildSuccess,
  handleError,
...
const module = {
  ...
  mutations: {
    ...
    update(
      ...
      let indexToUpdate = state.entities.findIndex(
        (el: GenericObject) => el._id == id,
      );
      state.entities.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state: GenericObject, id: string) {
      let indexToDelete = state.entities.findIndex(
        (el: GenericObject) => el._id == id,
      );
