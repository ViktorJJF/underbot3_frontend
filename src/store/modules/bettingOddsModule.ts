import api from '@/services/api/bettingOdds';
import {
  buildSuccess,
  handleError,
...
const module = {
  ...
  mutations: {
    ...
    create(state: GenericObject, data: GenericObject) {
      state.bettingOdds.push(data);
    },
    update(
      state: GenericObject,
      { id, data }: { id: string; data: GenericObject },
    ) {
      const indexToUpdate = state.bettingOdds.findIndex(
        (el: GenericObject) => el._id == id,
      );
      state.bettingOdds.splice(indexToUpdate, 1, {
        ...data,
      });
    },
    delete(state: GenericObject, id: string) {
      const indexToDelete = state.bettingOdds.findIndex(
        (el: GenericObject) => el._id == id,
      );
      state.bettingOdds.splice(indexToDelete, 1);
      state.total -= 1;
    },
  },
  getters: {},
